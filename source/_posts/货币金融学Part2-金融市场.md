...
2. 需求曲线的移动(其他条件不变):
   ...
   2. 物价上涨, money的需求整体增加, 曲线右移, 利率升高 (和预期通胀略有区别, 物价上涨的影响是永久的)
3. 供给曲线的移动(其他条件不变):
...
这一章介绍了利率的两个特性, 一个是风险特性, 风险越高, 利率补偿越高. 另外一个是久期特性, 久期越长, 一般利率补偿也越高.
...
[债券收益率曲线yield curve](/images/货币金融学/债券收益率曲线.jpg)

1. Expectations theory
   1. 假设长期利率就是短期利率期望的平均,
   2. 可以解释为什么在短期利率很低的时候长期利率会高, 短期利率很高的时候长期利率低
...
## 第七章: The stock market, the theory of rational expectations, and the efficient market hypothesis

这一章介绍了股票市场, 最重要的是股票的现值是未来现金流的折现, 其中的一个重要模型是股息贴现模型. 之后又介绍了有效市场假说.

### 7.1 Computing the price of common stock

核心: 股票现在的价值是股票未来的现金流的折现, 未来现金流包含两部分: 所有分红以及最后卖出的价格

1. Generalized Dividend Valuation Model(通用分红估值模型)

   $P_0=\frac{D_1}{(1+k_e)^1}+\frac{D_2}{(1+k_e)^2}+...+\frac{D_n}{(1+k_e)^n}+\frac{P_n}{(1+k_e)^n}$
   ...
   由于, 在n很大时, 上式的最后一项可以忽略不计, 或者说我们永久持有股票, 那么可以简化为:

   $P_0=\Sigma_t\frac{D_t}{(1+k_e)^t}$

2. The Gorden Growth Model(股息贴现模型)

   $P_0=\frac{D_1}{k_e-g}$

   其中:

   $g$: 未来股息的增长率(这个模型中把股息增长率看作一个定值)

   推导过程是上面的模型做了股息增长率不变的假设后, 近似得出的结果

### 7.2 How the market sets stock prices

内容不太复杂, 不同人对于预期收益率不同, 那么会导致每个人对于股票现值估算不同, 若只有一个卖家, 那买家将会是对预期收益率要求最低的那个. 有趣的是两个例子

1. 降息对股价的影响, 运用Gorden Growth Model

   1. 由于利率降低, 人们对于股票的预期收益率变低, $k_e$降低
   2. 由于降息会刺激经济, 因此$g$会增大

   综合下来, $P_0$上升, 股价上涨

2. 新冠对股价的影响, 运用Gorden Growth Model

   1. 经济受损, $g$会变小
   2. 风险增大, 人们对于股票的预期收益变高(风险补偿高了), $k_e$增大

   综合下来, $P_0$下降, 股价暴跌

### 7.3&7.4 Efficient market hypothesis

1. 有效市场假说:
   - 市场中证券的价格反映了可以获得的所有信息
   - 市场中所有未被发掘的机会会很快被聪明资金消除(这个理论也允许市场中的交易者不全了解信息页不全拥有理性的预期)

2. 随机漫步理论:
   - 证券的价格不可预测, 上涨和下跌的概率几乎一致
   - 它其实反映了有效市场假说, 因为一旦可以预测, 那么这个预测的值会被很快抹平
3. 一些应用
   - 公开报道的消息没什么卵用(已经在可获得的信息中了, 很多人比你早知道)
   - 追逐热点没什么用, 除非你是最早知道热点的一批人
   - 好消息并不一定带来股价的上涨, 要看与预期是否相符合

### 7.5&7.6

讨论了一些看似难以解释的市场现象, 和一些有效市场假说的争议点.
